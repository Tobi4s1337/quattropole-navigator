...
const components: Partial<Components> = {
	...
	p: ({ children, ...props }: React.HTMLAttributes<HTMLParagraphElement>) => (
		<p className="leading-6 [&:not(:first-child)]:mt-4 break-words overflow-wrap-anywhere" {...props}>
			{children}
		</p>
	),
	...
	a: ({
		...
		<a
			className="font-medium underline underline-offset-4 break-words overflow-wrap-anywhere max-w-full inline-block"
			target="_blank"
			rel="noreferrer"
			{...props}
		>
	...
	table: ({ children, ...props }: React.HTMLAttributes<HTMLTableElement>) => (
		<div className="my-6 w-full overflow-x-auto max-w-full">
			<table
				className="relative w-full border-none text-sm"
	...
	tr: ({ children, ...props }: React.HTMLAttributes<HTMLTableRowElement>) => (
		<tr className="last:border-b-none m-0 border-b" style={{ display: 'flex', flexWrap: 'wrap' }} {...props}>
			{children}
		</tr>
	),
	th: ({ children, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
		<th
			className="px-4 py-2 text-left font-bold break-words [&[align=center]]:text-center [&[align=right]]:text-right"
			style={{ minWidth: 'auto', flex: '1 1 auto' }}
			{...props}
		>
			{children}
		</th>
	...
	td: ({ children, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
		<td
			className="px-4 py-2 text-left break-words [&[align=center]]:text-center [&[align=right]]:text-right"
			style={{ minWidth: 'auto', flex: '1 1 auto' }}
			{...props}
		>
			{children}
...
const MemoizedMarkdownBlock = memo(
	({ content, className }: MarkdownBlockProps) => {
		return (
			<div className={cn("max-w-full overflow-x-hidden break-words", className)}>
				<ReactMarkdown
