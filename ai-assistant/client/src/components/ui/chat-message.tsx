...
const chatMessageContentVariants = cva("flex flex-col gap-2 max-w-full overflow-hidden", {
	variants: {
		variant: {
			default: "",
...
>(({ className, content, id: idProp, children, ...props }, ref) => {
	...
	return (
		<div
			ref={ref}
			className={cn(chatMessageContentVariants({ variant, type, className }), "max-w-full overflow-hidden")}
			{...props}
		>
			{content.length > 0 && (
